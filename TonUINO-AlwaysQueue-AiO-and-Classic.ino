...
#define DEBUG                // Zum Einsparen von Programmspeicher wird die Ausgabe
                               ...
                               // muss die #define DEBUG durch in Kommentar "//" setzen deaktiviert werden.
...
const uint8_t MenueVolume = 10;  // Bei Deaktivierung von MenueVol wird die Menülautstärke auf diesen Festwert eingestellt.
...
//#define LED_SR               // LED Animation mit Neopixel LED-Ring oder Strip
     #ifdef DEBUG            // Hardwareerweiterung erforderlich: (Neopixel Strip oder Ring)
...
static bool forgetLastCard = false;             // Variable zum Check Konfig/Adminmenü
...
void adminMenu(bool fromCard = false)
{
  ...
  #ifdef CardPause
    int subMenu = voiceMenu(10, 900, 900, false, false, 0, true); // 900- "Willkommen im Adminmenü,Wähle eine Funktion ...(mp3/ 900 - 910)
  #endif
  #ifndef CardPause
  ...
  else if (subMenu == CreateFolderCards)                                      // 902- "Einzelkarten für Ordner festlegen
  {
  ...
  else if (subMenu == CreateModifier)                                        // 903- "Modifikationskarte erstellen
  {
  ...
  else if (subMenu == SetupShortCuts)                                      // 904-"Shortcut configurieren ( mit Weckershortcut)
  { // 995-"Shortcut configurieren ( ohne Weckershortcut)
    ...
    uint8_t shortcut = voiceMenu(4, 942, 942);                // (943-946)    Playtaste/UpTaste/DownTaste/WelcomeSound
#endif
#ifdef Wecker                                                 // wenn Weckerfunktion definiert
    uint8_t shortcut = voiceMenu(5, 942, 942);                // (943-947)    Playtaste/UpTaste/DownTaste/WelcomeSound/WeckerSound
#endif
#endif

#ifdef LastCard                                               // wenn Letzte Karte auf PlayTaste legen definiert
#ifndef Wecker                                                // wenn Weckerfunktion nicht definiert
    uint8_t shortcut = voiceMenu(3, 942, 943);                // (942-944)     UpTaste/DownTaste/WelcomeSound
#endif
#ifdef Wecker                                                 // wenn Weckerfunktion definiert
    uint8_t shortcut = voiceMenu(4, 942, 943);                // (942-945)     UpTaste/DownTaste/WelcomeSound/WeckerSound
#endif
#endif
  ...
  else if (subMenu == InvertButtons)                                            //905- "Funktion der Lautstärketasten umdrehen
  {
    int temp = voiceMenu(2, 932, 932, false);                       //932- "Möchtest du die Funktion der Lautstärketasten umdrehen?
    //Du musst dann die Tasten lange drücken um ein Lied vor oder zurückzugehen.
    if (temp == 2)
    {
      mySettings.invertVolumeButtons = true;                        //933- "Ja
    }
    else
    {
      mySettings.invertVolumeButtons = false;                       //934- "Nein
    }
  }
  ...
  else if (subMenu == VolumeEQ)                                            // 906- "Einstellungen für die Lautstärke und den Klangcharakter ändern
  {
#ifdef EarPhone
    int VolMenu = voiceMenu(3, 911, 911, false, false, 0, true);      // (mp3/ 912 - 915)
                                                                    // 911 - Bitte wähle mit den LS Tasten die Einstellung aus und bestätige mit der Pausetaste
                                                                    // 912 - Einstellungen für die Lautstärke des Lautsprechers  ändern
                                                                    // 914 - Einstellungen für die Lautstärke des Kopfhörers ändern
                                                                    // 913 - Den Klangcharakter konfigurieren
#endif
#ifndef EarPhone
    int VolMenu = voiceMenu(2, 911, 911, false, false, 0, true);      // (mp3/ 912 - 913)
                                                                      // 911 - Bitte wähle mit den LS Tasten die Einstellung aus und bestätige mit der Pausetaste
                                                                      // 912 - Einstellungen für die Lautstärke des Lautsprechers  ändern
                                                                      // 913 - Den Klangcharakter konfigurieren
#endif
    ...
    {
      int LspMenu = voiceMenu(3, 915, 915, false, false, 0, true);      //(mp3/ 915- 918)
                                                                    // 915 - Bitte wähle mit den LS-Tasten die zu ändernde Lautstärke aus
                                                                    //       und bestätige mit der Pausetaste
                                                                    // 916 - Maximale Lautstärke Lautsprecher
                                                                    // 917 - Minimale Lautstärke Lautsprecher
                                                                    // 918 - Lautstärke des Lautsprechers beim Start

      // ********** Max Lautstärke des Lautsprechers *******************

      if (LspMenu == 1)                                                       // 916 - Maximale Lautstärke Lautsprecher
      {
        AdjVol = true;
        mySettings.maxVolume = voiceMenu(30 - mySettings.minVolume, 935, mySettings.minVolume,  //935 - Bitte wähle mit den LS-Tasten die
      ...
      if (LspMenu == 2)                                                                 // 917 - Minimale Lautstärke Lautsprecher
      {
        AdjVol = true;
        mySettings.minVolume = voiceMenu(mySettings.maxVolume - 1, 936, 0,              // 936 - Bitte wähle mit den LS-Tasten die
                                         false, false, mySettings.minVolume);            //      min. Lautstärke des Lautsprechers aus
      }

      // ********** Initial Lautstärke des Lautsprechers *******************

      if (LspMenu == 3)                                                       // 918 - Lautstärke des Lautsprechers beim Start
      {
        AdjVol = true;
        mySettings.initVolume = voiceMenu(mySettings.maxVolume - mySettings.minVolume + 1, 937, // 937 - Bitte wähle mit den LS-Tasten die
...
#ifdef EarPhone     
    if (VolMenu == 2)                                                         // 914- "Einstellungen für die Lautstärke des Kopfhörers ändern

    {
      int EarMenu = voiceMenu(3, 919, 919, false, false, 0, true);              //  (mp3/ 918 - 921)
                                                                              // 919 - Bitte wähle mit den LS-Tasten die zu ändernde Lautstärke aus
                                                                              //       und bestätige mit der Pausetaste
                                                                              // 920 - Maximale Lautstärke Kopfhörer
                                                                              // 921 - Minimale Lautstärke Kopfhörer
                                                                              // 922 - Lautstärke des Kopfhörers beim Start

      // ********** Maximale Lautstärke Kopfhörer **************************

      if (EarMenu == 1)                                                       // 920 - Maximale Lautstärke Kopfhörer
      {
        AdjVol = true;
        mySettings.maxEarVol = voiceMenu(30 - mySettings.minEarVol, 938,        //938 - Bitte wähle mit den LS-Tasten die
      ...
      if (EarMenu == 2)                                                           // 921 - Minimale Lautstärke Kopfhörer
      {
        AdjVol = true;
        mySettings.minEarVol = voiceMenu(mySettings.maxEarVol - 1, 939,           // 939 - Bitte wähle mit den LS-Tasten die
                                         0, false, false, mySettings.minEarVol);  //       min. Lautstärke des Kopfhörers aus
      }

      // ********** Initial Lautstärke Kopfhörer **************************

      if (EarMenu == 3)                                                         // 922 - Lautstärke des Kopfhörers beim Start
      {
        AdjVol = true;
        mySettings.initEarVol = voiceMenu(mySettings.maxEarVol - mySettings.minEarVol + 1, 940, // 940 - Bitte wähle mit den LS-Tasten die
    ...
    if (VolMenu == 2)                                                         //913- "Den Klangcharakter konfigurieren

    {
      mySettings.eq = voiceMenu(6, 924, 924, false, false, mySettings.eq);   //924- "Bitte wähle eine Einstellung für den Klangcharakter
  ...
  else if (subMenu == SetupStandbyTimer)                                    // 907- "StandbyTimer Konfigurieren
  {
  ...
  else if (subMenu == LockAdminMenu)                                    // 908- "Admin Menü schützen
  {
  ...
  else if (subMenu == ResetEEPROM)                                    // 909- "Tonuino zurücksetzen
  {
#ifdef DEBUG
...
// ********** Tonuino StopWhenCardAway ****************
#ifdef CardPause
  else if (subMenu == StopWhenCardAway) {
    switch (voiceMenu(2, 948, 932, false)) {
      case 1:
        mySettings.stopWhenCardAway = true;
        break;
      case 2:
